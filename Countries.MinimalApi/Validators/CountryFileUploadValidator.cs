...
public class IFormFileValidator : AbstractValidator<IFormFile>
{
    ...
    {
        RuleFor(x => x).Must((file, context) => { return file.ContentType == "text/csv"; })
            .WithMessage("The Content-Type is not valid");

        RuleFor(x => x).Must((file, context) => { return file.FileName.EndsWith(".csv"); })
        ...
        RuleFor(x => x).Must((file, context) =>
            {
                var exeSignatures = new List<string>
                    { "4D-5A", "5A 4D" }; // string representation of hexadecimal signature of an execute file
...
public class CountryFileUploadValidator : AbstractValidator<CountryFileUpload>
{
    ...
    {
        RuleFor(x => x.File).Must((file, context) => { return file.File.ContentType == "text/csv"; })
            .WithMessage("ContentType is not valid");

        RuleFor(x => x.File).Must((file, context) => { return file.File.FileName.EndsWith(".csv"); })
            .WithMessage("The file extension is not valid");
        ...
        RuleFor(x => x.AuthorName).NotEmpty().WithMessage("{PropertyName} is required")
            .Custom((authorName, context) =>
            {
                ...
                if (rg.Matches(authorName).Count > 0)
                    // Raises an error
                    context.AddFailure(new ValidationFailure("AuthorName",
